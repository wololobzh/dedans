...
function whereFor(filter: CampusFilter): Record<string, unknown> {
  const where: Record<string, unknown> = {};
  if (filter.campusIds !== undefined) where.id = { in: filter.campusIds };
  if (filter.status !== 'all') where.status = filter.status;
  return where;
}

function excludesCampus(filter: CampusFilter, id: string): boolean {
  return filter.campusIds !== undefined && !filter.campusIds.includes(id);
}

export class PrismaCampusRepository implements CampusRepository {
  constructor(private readonly db: Database) {}

  async list(filter: CampusFilter): Promise<Campus[]> {
    const rows = await this.db.campus.findMany({ where: whereFor(filter), orderBy: [{ name: 'asc' }, { id: 'asc' }] });
    return rows.map((row) => mapCampus(row));
  }

  async findById(id: string, filter: CampusFilter): Promise<Campus | null> {
    if (excludesCampus(filter, id)) return null;
    const row = await this.db.campus.findFirst({ where: { ...whereFor(filter), id } });
    return row ? mapCampus(row) : null;
  }

  async create(input: NewCampus): Promise<Campus> {
    try {
      return mapCampus(await this.db.campus.create({ data: input }));
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') throw new DuplicateCampusError('Campus already exists');
      throw error;
    }
  }

  async update(id: string, changes: CampusChanges, filter: CampusFilter): Promise<Campus | null> {
    if (excludesCampus(filter, id)) return null;
    try {
      const result = await this.db.campus.updateMany({ where: { ...whereFor(filter), id }, data: changes });
  ...
  async deactivate(id: string, actorId: string, reason: string, filter: CampusFilter): Promise<Campus | null> {
    if (excludesCampus(filter, id)) return null;
    const result = await this.db.campus.updateMany({
      where: { ...whereFor(filter), id, status: 'active' },
      data: { status: 'inactive', deactivatedAt: new Date(), deactivatedBy: actorId, deactivationReason: reason },
    });
    if (result.count === 0) return null;
    return this.findById(id, { ...filter, status: 'inactive' });
  }
}
...
export class PrismaAuditEventRepository implements AuditEventRepository {
  ...
  async append(event: AuditEventInput): Promise<void> {
    const snapshot = (value: Campus | null): Prisma.InputJsonValue | typeof Prisma.JsonNull =>
      value === null ? Prisma.JsonNull : (JSON.parse(JSON.stringify(value)) as Prisma.InputJsonValue);
    await this.db.auditEvent.create({
      data: {
        ...
        authorizedCampusIds: event.authorizedCampusIds === null ? Prisma.DbNull : [...event.authorizedCampusIds],
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        return await this.db.$transaction(
          async (tx) =>
            operation({
              campuses: new PrismaCampusRepository(tx),
              auditEvents: new PrismaAuditEventRepository(tx),
              dependencyChecker: new PrismaCampusDeactivationDependencyChecker(tx),
            }),
